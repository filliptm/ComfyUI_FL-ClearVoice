"""
Snake Activation Functions for NovaSR
Based on BigVGAN's learnable periodic activations.
"""
...
def snake_fast_inference(x: Tensor, a: Tensor, inv_2b: Tensor) -> Tensor:
    """JIT-compiled fast Snake activation for inference."""
...
class Snake(nn.Module):
    ...
    def prepare(self):
        """Pre-compute values for fast inference."""
        with torch.no_grad():
...
class SnakeBeta(nn.Module):
    """
    Snake activation with separate alpha (frequency) and beta (amplitude) parameters.

    Formula: x + (1 - cos(2*a*x)) / (2*b)
    """
