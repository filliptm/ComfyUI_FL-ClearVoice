"""
Common utility functions for NovaSR.
"""
...
def get_padding(kernel_size, dilation=1):
    """Calculate padding for same-size convolution output."""
    return int((kernel_size * dilation - dilation) / 2)


def convert_pad_shape(pad_shape):
    """Convert pad shape to F.pad format."""
    l = pad_shape[::-1]
...
def slice_segments_audio(x, ids_str, segment_size=4):
    """Slice audio segments from tensor."""
    ret = torch.zeros_like(x[:, :segment_size])
    for i in range(x.size(0)):
        idx_str = ids_str[i]
        idx_end = idx_str + segment_size
...
def get_timing_signal_1d(length, channels, min_timescale=1.0, max_timescale=1.0e4):
    """Generate 1D timing signal (positional encoding)."""
...
def subsequent_mask(length):
    """Create subsequent mask for attention."""
...
def shift_1d(x):
    """Shift tensor by 1 along time dimension."""
