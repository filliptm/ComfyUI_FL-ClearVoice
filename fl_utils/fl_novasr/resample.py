"""
Resampling modules for NovaSR.
Polyphase upsampling and downsampling with Kaiser-windowed sinc filters.
"""
...
def kaiser_sinc_filter1d(cutoff, half_width, kernel_size):
    """
    Create a Kaiser-windowed sinc filter for resampling.
    ...
    Returns:
        Filter coefficients of shape [1, 1, kernel_size]
    """
...
def _polyphase_upsample_fused(x: Tensor, weight: Tensor, ratio: int):
    """
    JIT-compiled polyphase upsampling.

    Uses efficient polyphase decomposition for 2x upsampling with 12-tap filter.
    """
    # Padding for 12-tap center alignment
    x = F.pad(x, (2, 3))
    ...
    # Slice for 12-tap alignment
    return out[..., 2:-2]


class UpSample1d(nn.Module):
    """
    Polyphase upsampling module.

    Uses Kaiser-windowed sinc filter for high-quality upsampling.
    """
    ...
    def prepare(self):
        """Pre-compute polyphase decomposition for fast inference."""
        with torch.no_grad():
...
class LowPassFilter1d(nn.Module):
    ...
    def prepare(self):
        """Pre-compute filter expansion for fast inference."""
        with torch.no_grad():
    ...
    def forward(self, x: Tensor):
        if not self._prepared and not self.training:
            self.prepare()
        C = x.shape[1]
        # Padding 5 for 12-tap kernel
        return F.conv1d(x, self.f_opt[:C], stride=self.stride, padding=5, groups=C)


class DownSample1d(nn.Module):
    """
    Downsampling module with anti-aliasing low-pass filter.
    """
