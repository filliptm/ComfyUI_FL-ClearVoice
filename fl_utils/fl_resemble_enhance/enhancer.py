"""
Enhancer model - extracted from resemble_enhance without deepspeed dependency.
"""
...
def load_denoiser(run_dir, device):
    """Load denoiser model without going through train.py."""
    if run_dir is None:
        hp = DenoiserHParams()
    else:
...
# Import components from the package that don't chain to train.py
from resemble_enhance.common import Normalizer
from resemble_enhance.melspec import MelSpectrogram
from resemble_enhance.enhancer.lcfm import CFM, IRMAE, LCFM
...
class Enhancer(nn.Module):
    ...
    def _load_pretrained(self, path):
        ...
        self.lcfm.cfm.load_state_dict(cfm_state_dict)
    ...
    def _may_denoise(self, x: Tensor, y: Tensor = None):
    ...
    def forward(self, x: Tensor, y: Tensor = None, z: Tensor = None):
        """
        Args:
        ...
        x_mel_original = self.normalizer(self.to_mel(x), update=False)
        ...
        y_mel = _maybe(self.to_mel)(y)
        y_mel = _maybe(self.normalizer)(y_mel)

        lcfm_decoded = self.lcfm(x_mel_denoised, y_mel, ψ0=x_mel_original)
